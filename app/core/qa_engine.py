...
class LectureQA:
    ...
    def answer_question(self, question: str, top_k: int = RETRIEVAL_TOPK) -> Dict:
        prepared = self._prepare_answer_context(question, top_k)
        if prepared is None:
            return {
...
def _load_lecture_resources(lecture_id: str):
    ...
    if cached is not None:
        emb, segments, bm25 = cached
        return emb.copy(), copy.deepcopy(segments), bm25
    ...
    with _lecture_cache_lock:
        if len(_lecture_cache) >= 8:
            _lecture_cache.pop(next(iter(_lecture_cache)))
        _lecture_cache[lecture_id] = (emb, segments, bm25)
