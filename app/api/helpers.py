...
def build_qa_redirect(lecture_id: str):
    lecture_meta = load_lecture_metadata(lecture_id)
    src_url = lecture_meta.get("source_url", "")
    return redirect(url_for("qa.qa_view", lecture_id=lecture_id, src_url=src_url))
...
def flash_generation_failure(label: str, exc: Exception) -> None:
    if is_rate_limit_error(exc):
        flash(
            f"{label} failed: API rate limit reached. Please wait 30-60 seconds and try again.",
            "error",
        )
        return
